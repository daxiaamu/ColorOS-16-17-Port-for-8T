...
不要在加密初始化前手动填充 /data/app，也不要提前创建影响首次安装判定的应用目录。包不得格式化 data/metadata。安装后再次格式化会删掉 reserve；恢复出厂后资源也会丢失，需重新刷完整包或使用应用找回。刷前需要检查 data 可读写、容量与 RAM，不能写完系统才发现预装放不下。
