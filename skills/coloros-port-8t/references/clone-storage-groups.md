...
OS17 新建第七个 Via 分身复现同类错误：首次 FUSE ready=1，随后 Android/data bind EACCES、卷 UNMOUNTABLE、外部存储未就绪阻止启动。单独重启 MediaProvider 获得新用户附加组后恢复，不能把后续 ready=0 误认成第一原因。

在当前 OS17 方法中重新定位创建成功、updateCloneUsers 之后、startProfile 之前的调用点；仍限定 KB2000，解析媒体 UID，通过 AMS.killUid 刷新。保留 dev57 QQ 修复所在 DEX，不能拿 OS16 整 JAR 覆盖。持久修复后再建一个新用户，实测组已更新、卷 mounted、真实桌面入口无需重启可到隐私欢迎页。未接受应用条款，测试分身经原生 UI 清理，未修改主应用数据。

这证明本次新增用户路径，不代表 20 个分身和并发媒体写入均通过。该增量已刷入，未合入后继完整 ZIP。
