...
保持显示类型、原生 AOD 亮度请求与退出状态成套匹配。仅改全天支持、强制 GPU、补写亮度都曾无效；固定状态栏 alpha 已撤回。后续AMB655X配套内核/SystemUI改善了过渡；现行双模式方案及两阶亮度限制见 [aod-transition](aod-transition.md)，不能通过随意延时宣布全部修复。
