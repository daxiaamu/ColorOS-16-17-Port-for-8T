...
dev60 是历史候选，不能覆盖当前状态。dev64 将同一 CameraManager 条件修复合到当时最新 system，保留热点兼容；原厂 17.1.13 APK 的预览截图与持续增长的帧计数通过，QQ 和原生相机预览回归通过。dev65 JPEG 修复后又重复验证扫一扫预览。未扩大为所有识别模式均验收。

退役 APK 实验必须同时检查只读预装和 Data 更新。Data 中原版遮住旧补丁不代表底层已清理；卸载更新后可能复现。干净发布输入使用原版预装，dev67 已按旧镜像哈希恢复实验涉及的 my_product/my_stock，整镜像读回通过；重启后确认原版预装与 Data 中原版哈希一致、旧路径消失、持续出帧且截图有真实画面。不能以 pm path 单项代替分区证据。
