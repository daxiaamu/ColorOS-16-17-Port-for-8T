...
ARGS=(O="$ROOT/build/out" ARCH=arm64 REAL_CC="ccache clang" LLVM=1 LLVM_IAS=0 CROSS_COMPILE=aarch64-linux-gnu- CROSS_COMPILE_ARM32=arm-linux-androideabi- CLANG_TRIPLE=aarch64-linux-gnu- LOCALVERSION=+ OPLUS_FEATURE_PROCESS_RECLAIM=yes OPLUS_FEATURE_MEMLEAK_DETECT=yes OPLUS_FEATURE_UFS_SHOW_LATENCY=yes OPLUS_FEATURE_PADL_STATISTICS=yes OPLUS_FEATURE_UFSPLUS=yes OPLUS_FEATURE_SECURE_ROOTGUARD=no OPLUS_FEATURE_SECURE_MOUNTGUARD=no OPLUS_FEATURE_SECURE_EXECGUARD=no)
