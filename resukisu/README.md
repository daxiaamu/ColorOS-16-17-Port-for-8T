...
> Device test status (2026-09-12): the initial kernel causes an 8T startup failure. Restoring the original boot restores normal boot. The diagnostic kernel reached Android but loaded no vendor modules, and SystemServer watchdog traces show startup blocking. The original OEM module certificate was missing, and 195 imported kernel symbol CRCs differ from the vendor modules. The certificate is now included in the build configuration; ABI correction and a successful device retest are still pending. Actions now blocks packaging when the OEM certificate or vendor ABI check fails. Do not flash earlier artifacts as a working release.
