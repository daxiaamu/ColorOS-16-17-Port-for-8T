...
fixes=[
 # Keep the packed union definition; genksyms cannot parse packed return types.
 (K/'drivers/platform/msm/gsi/gsi.c','static union __packed gsi_channel_scratch __gsi_update_mhi_channel_scratch(','static union gsi_channel_scratch __gsi_update_mhi_channel_scratch(',1),
...
run(str(K/'scripts/config'),'--file',str(W/'out/.config'),'-e','KSU','-e','KSU_MANUAL_HOOK','-d','KSU_TRACEPOINT_HOOK','-e','KSU_MANUAL_HOOK_AUTO_SETUID_HOOK','-e','KSU_MANUAL_HOOK_AUTO_INITRC_HOOK','-e','KSU_MANUAL_HOOK_AUTO_INPUT_HOOK','-e','KSU_MULTI_MANAGER_SUPPORT','-d','KSU_SUSFS','-d','KSU_DEBUG','-e','OPLUS_BINDER_ASYNC_UX_COMPAT')
